...
func attempt_to_connect():
	...
	if world_node:
		...
		if player and container:
			...
			player.staminaChanged.connect(Callable(self, "update"))
			update()
...
func _on_node_removed(node):
	if node == get_node(world_node_path):
		...
		if player:
			player.staminaChanged.disconnect(Callable(self, "update"))
...
func update():
	if player and container:
		value = player.currentStamina * 100 / container.playerMaxStamina
	else:
		value = 0
